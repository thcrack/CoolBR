...
export class Chatroom extends Component {
    ...
    componentDidMount() {
        ...
        this.connection.on("ReceiveMessage",
            (user, message) => this.appendMessage(user, message));
        this.connection.on("ReceiveMessages", (msgs) => {
            msgs.forEach(msg => this.appendMessage(msg.user, msg.content));
        });
        this.connection.start().then(async () => {
            await this.connection.invoke("GetHistoryMessages")
                .then(() => this.setState({chatroomReady: true}));
        });
